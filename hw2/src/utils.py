...
def downsample(data, scale):
    ...
    if scale not in {4,2,1} :
        raise ValueError('the downsample scale is not in {4,2,1}')
    if scale == 4:
        return data
    return data[::3-scale, ::2]

def upsample(data, scale):
    '''
        用現有值填充downsample後消失的部分
    '''
    if scale not in {4,2,1} :
        raise ValueError('the downsample scale is not in {4,2,1}')
    if scale == 4:
        return data
    return data.repeat(3-scale,axis=0).repeat(2,axis =1)

def quantize(data, data_type, quality, inverse):
    '''
        量化
    '''
    if data_type == 'Y':
    ...
    qf = 5000/quality if quality < 50 else 200-2*quality
    if qf == 0:
        return data
    #print(qf, quality)
    newtable = (QT*qf)/100
    return data/newtable if not inverse else data*newtable

def dct2d(data):
    '''
        dct2d
    '''
    return dct(dct(data, norm='ortho', axis=0), norm='ortho', axis=1)

def idct2d(data):
    '''
        2d idct
    '''
    return idct(idct(data, norm='ortho', axis=0), norm='ortho', axis=1)

def block_slice(data, rows, cols):
    '''
    把影像分成很多的8*8方塊
    '''
    h, _ = data.shape
    return (data.reshape(h//rows, rows, -1, cols)
            .swapaxes(1, 2)
            .reshape(-1, rows, cols))
